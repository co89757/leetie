...
int longest_file_path(const string& input) {
  unordered_map<int, int>
      pathlen;  // mapping from indent-level(depth) to path basename length
  pathlen[0] = 0;
  int maxlen = 0;
  char* s = strdup(input.c_str());
  char* line = strtok(s, "\n");
  while (line) {
    int depth = strspn(line, "\t");
    string name = string(line + depth);
    if (name.find('.') != string::npos) {
      // this is a file (leaf), update max file path length
      int newmax = pathlen[depth] + name.size();
      maxlen = maxlen > newmax ? maxlen : newmax;
    } else {
      // get pathlen for next level
      pathlen[depth + 1] = 1 + pathlen[depth] + name.size();
    }
    // go to next line
    line = strtok(NULL, "\n");
  }
...
int main() {
  string s1(
      "dir\n\tsubdir1\n\t\tfile1."
      "ext\n\t\tsubsubdir1\n\tsubdir2\n\t\tsubsubdir2\n\t\t\tfile2.ext");
  int actual = longest_file_path(s1);
  ASSERT(actual == 32, "expected=32; actual=%d", actual);
}